...
exports = module.exports = class mongodb_Collection {
    ...
    collection(callback){
        ...
        if(typeof this._Mongodb == 'object'){
            this._MongoColl = this._Mongodb.collection(this._CollName);
            return callback(null,this._MongoColl);
        }
        else if(typeof this._Mongodb == 'function'){
            this._Mongodb(function(err,ret){
                if(err){
                    return callback(err,ret);
                }
                self._MongoColl = ret.collection(self._CollName);
                return callback(null,self._MongoColl);
            });
        }
        else{
            return callback('MongoError','Mongodb illegal ');
        }
    }
    ...
    unset(id,key,callback) {
        var query  = this.query(id);
        var update = {"$unset":this.values(key,1),"multi":this.isMultiWrite(id)};
        var option = {};
        this.update(query,update,option,callback);
    }
    ...
    page(query, page, size, sort, option, callback){
        ...
        var find = function(coll){
            ...
            coll.find(query, option, function(err,cursor){
                ...
                getArrFromCursor(cursor,result );
            });
        }
    ...
    update(query,update,option,callback,ErrorReNum) {
        if(this._MultiColl){
            return this._MultiColl.update(query,option,callback);
        }
...
function getObjFromCursor(cursor,options,callback){
    var key = options['DataKey'] || '_id';
    var DataParse = options['DataParse'] || null;
    var rows = {};
