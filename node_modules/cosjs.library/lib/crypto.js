...
//session 加密
function session(secret,garbled){
    if (!(this instanceof session)) {
        return new session(secret,garbled)
    }
...
session.prototype.encode = function sessionEncode(str){
    var garbledStr = '';
    if(this.garbled){
        garbledStr = exports.md5(Date.now().toString()).substr(0,this.garbled);
    }
...
session.prototype.decode =  function sessionDecode(str){
