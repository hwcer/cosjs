    ...
    get(id) {
        ...
        this.redis.get(id,key,function(err,ret){
            if(err){
                ...
                getDataFromMongodb.call(self,id,key,function(e1,r1){
                    if(e1 || !r1) {
                        return callback(e1,r1);
                    }
                    else{
                        return callback(null,mergeObject(r1,key));
                    }
                });
            }
        });
...
//id,key,[val,callback]
    ...
    del(ids) {
        ...
        if(!key){
            this.mongodb.del(ids,DBDelResult);
        }
        else{
...
function mget(ids,key,callback){
    if(ids.length > this.limit ){
        ...
        return this.mongodb.get(ids,key,options,callback);
    }
...
function getDataFromMongodb(id,keys,callback){
    var options = {'dataType':'json'};
    this.mongodb.get(id,null,options,(err,ret)=>{
        if(err || !ret){
            return callback(err,ret);
        }
        if(Array.isArray(id)){
