    ...
    get(id) {
        ...
        this.redis.get(id,key,function(err,ret){
            if(err){
                ...
                getDataFromMongodb.call(self,id,key,null,callback);
            }
            else{
...
function mget_result(data,err,ret){
    ...
    getDataFromMongodb.call(this,data['arr'],data['key'],
        (k,v)=>{ data['rows'][k] = v; },
        (e,r)=>{
            if(e) { return callback(e,r); } else { return callback(null,data['rows']); }
        },
    );
}
...
function getDataFromMongodb(id,keys,parse,callback){
    var options = {'dataType':'json','dataParse':getDataFromMongodbParse.bind(this,parse)};
    this.mongodb.get(id,null,options,callback);
}

function getDataFromMongodbParse(parse,key,val) {
    addDataToRedis.call(this,key,val);
