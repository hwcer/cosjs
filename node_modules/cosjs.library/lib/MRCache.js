...
function mget(ids,key,callback){
    ...
    var rows = {},
        worker = mget_worker.bind(this,rows,key),
        result = mget_result.bind(this,rows,callback);
    var mgetTask = new task(ids,worker,result);
    mgetTask.breakOnError = true;
    mgetTask.asyn();
}

function mget_worker(rows,key,id,callback){
    this.get(id,key,function(err,ret){
        if(err){
            return callback(err,ret);
        }
        if(ret){
...
function mget_result(rows,callback,err,ret){
    if(err){
        callback(ret[0]);
    }
    else{
...
function addDataToRedis(id,key,val) {
    ...
    if( typeof key == 'object' ){
        if(key['_id'] && typeof key['_id'] == 'object') {
