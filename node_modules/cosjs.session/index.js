...
session.prototype.get = function (key,type) {
    if(!this.userData || !(key in this.userData) ){
        return null;
    }
...
function session_start(req,res,callback){
    this.redis = redis(this.option.redis,this.option.prefix,this.option.format);
    this.redis.multi();
    if(!this.level){
...
function session_create(req,res,uid,data,callback) {
    var self = this,sid;
    if(this.option.ObjectID){
        this.uid = sid = crypto.ObjectID();
    }
    else{
        this.uid = uid;
        sid = crypto.encode(uid,self.option.secret);
    }
    this.sid = sid;
    ...
    self.redis.set(self.uid,newData,session_callback);
    if(self.option.expire){
        self.redis.expire(self.uid,self.option.expire,session_callback);
    }
...
function session_reset(){
    ...
    this.redis.expire(this.uid,this.option.expire,session_callback);
    this.redis.save(session_callback);
}
