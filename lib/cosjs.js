...
global.cosjs={};
//数据库表结构多重排序
cosjs.sort = function(arr,key,order){
    if(!key){
        return false;
    }
    var orderKeys={};
...
cosjs.timeFormat = function(str,time){
    var date;
    if(time===undefined){
        date = new Date();
    }
...
cosjs.dataFormat=function (val, type) {
...
//同步/异步 批量执行任务
cosjs.task = function(tasks,worker,callback){
...
cosjs.callback = function(err,ret){
    if(err){
        return false;
    }
    else{
...
cosjs.redis = function(config, type, callback){
    ...
    if(type){
        if(redis[type]){
            ...
            throw new Error("$.redis arguments[1] error");
        }
    }
    else{
...
cosjs.mongo = function(config,dbName,collName,callback){
...
cosjs.config = function(name, key, dir) {
    var file = [config['root'],dir || config['share'] + '/config',name].join('/');
    try{
...
cosjs.loader = function (name,dir){
