...
var coll=function(poolKey,collName) {
    ...
    //错误日志
    var ErrorLogs = function(method,query,update,option,err){
        if(self.ErrorLogs){
            console.error(new Date().toLocaleString(),  'mongodb',method,JSON.stringify(query),JSON.stringify(update),JSON.stringify(option),err['code']||0,err['errmsg']||err['message']||'');
        }
    }
    ...
    this.add=function(data,option,callback,ErrorReNums) {
        ...
        var result = function(err,ret){
            if(!err){
                ...
                ErrorLogs('add',null,data,option,err);
            }
            else{
                setTimeout(function(){
    ...
    this.remove=function(query,option,callback,ErrorReNums) {
        ...
        var result = function(err,ret){
            if(!err){
                ...
                ErrorLogs('remove',query,null,option,err);
            }
            else{
                setTimeout(function(){
    ...
    this.update = function(query,update,option,callback,ErrorReNums) {
        if(option['multi'] && option['upsert']){
            throw new Error('mongodb update option.multi and option.upsert is unreliable');
        }
        if(!ErrorReNums){
            ErrorReNums=0;
        }
        ...
        var result = function(err,ret){
            if(!err){
                ...
                ErrorLogs('update',query,update,option,err);
            }
            else{
                setTimeout(function(){
