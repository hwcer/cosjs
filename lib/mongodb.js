...
mongoColl.prototype.util.query = function(keys,parse,fk){
    var query = {};
    if(!keys){
        return query;
    }
...
mongoColl.prototype.util.fields = function (keys) {
    var fields = {};
    if(!keys){
        return fields;
    }
...
mongoColl.prototype.util.values = function (key,val) {
    var value = {};
...
mongoColl.prototype.util.multi = function(id){
...
mongoColl.prototype.util.ObjectID =function(keys,parse){
    ...
    var updateCache = function(cmd,id,key,val,MRet,callback){
        ...
        redis[cmd](id, key, val, function (err, ret) {
            if (!err && self.expire > 0) {
                redis.expire(id, self.expire, $.callback);
            }
            callback(err, ret);
        });
    }
