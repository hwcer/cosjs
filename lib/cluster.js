...
exports.fork = function(name,handle){
    ...
    var handleConfig = {  "id": 0, "name":name, "handle": handle,"handleArgs": handleArgs,"handleMessage":handleMessage.bind(this) };
...
function subscribe(arr){
    ...
    process.on(event,config['handleMessage']);
}
...
function handleMessage(arr) {
    ...
    var worker = cluster.workers[this.id];
    if(worker){
