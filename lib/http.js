...
var http=function() {
    ...
    this.start = function () {
        if (self.config['workers'] == 0) {
            self.config['workers'] = require('os').cpus().length;
        }
        var fork = require('./fork');
        fork.set(ceateHttpServer,self.config['workers']);
        fork.start();
    }
    ...
    function apiLoader(req, res, apiArrPath) {
        ...
        var $modReq = $.loader($modApi,$modDir);
