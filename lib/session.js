...
exports = module.exports = function(app,req, res){
    return new session(app,req, res);
}


var session = function (app, req, res) {
    var self = this,
        secret = app.get('secret') || Math.random().toString(),
        crypto = new _crypto(secret),
        isLock = false, delayNum = 0,
        expires = app.get('session expires'),
        lock_key  = '_session_lock',       //进程锁Key
        oauth_key = '_session_oauth';

    this.id = '';                             //session id
    this.key = '';                            //user id,cache key
	this.lock = app.get('session lock');    //[num,ms]
    this.oauth = 0;
    this.dataset = library.dataset();
	
    var conn;

    var start = function(){
        var dtype = app.get('session dtype');
        var dbase = app.get('session dbase');
        var config = typeof dbase == 'function' ? dbase(req,res) : dbase;
        switch (dtype){
            case "redis":
                conn = new _redis(config);
                break;
            case "mongo":
                conn = new _mongo(config);
                break;
            case "object":
                conn = config;
                break;
        ...
        var session_id = app.get('session id') || '_id';
        self.id = req['get'](session_id) || '';
    ...
    //获取并检查数据有效性
    var getData = function (keys,callback,reqKey) {
        this.oauth = 1;

        if( !self.id ){
            return callback('login_failed','session_id_empty');
        }
        if( !self.key ){
            return callback('login_failed','session_key_empty');
        }

        var getReqKey = function() {
            if (!keys) {
                reqKey = null;
            }
        ...
        var lockDelay = function(){
            delayNum ++;
            if( delayNum > self.lock[0] ){
                callback("session_busy");
            }
            else{
                setTimeout(lockCheck,self.lock[1]);
            }
        }
        ...
        var result = function(){
            ...
            if( delayNum > 0 ){
                //数据失去时效,其他程序已经修改过session,重新获取session数据
                getData(keys,callback,reqKey);
            }
            else{
                callback(null, self.dataset.get(keys) );
            }
        }
