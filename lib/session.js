...
var session = function (req, res) {
    ...
    var start = function(){
        var dbase = typeof config['dbase'] == 'function' ? config['dbase'](res['cosjs']) : config['dbase'];
        switch (config['dtype']){
            case "redis":
    ...
    var getData = function (keys,callback) {
        ...
        conn.get(self.key,reqKey,function(err,ret){
            ...
            //续约
            if(config['expires']){
                var expires = config['expires'] + nowtime;
...
var _crypto = function(secret){
    ...
    this.encode = function(str) {
        ...
        var code = $.md5($.roll(1000,9999).toString()).substr(0,secrlen);
        var skey = $.md5(code + this.secret);
...
var _redis = function (dbase) {
    if(!dbase){
        throw new Error("[session] redis config dbase empty") ;
    }
...
var _mongo = function (dbase) {
    if(!dbase){
        throw new Error("[session] mongo config dbase empty") ;
    }

    var conn = $.mongo(dbase,null,'cookie');
    ...
    this.incr = function (id, key, val, callback) {
        ...
        conn.incr(query, update, option, function(err,ret){
            if(err){
                ...
                return retValue(key,ret,callback);
            }
        });
    }
